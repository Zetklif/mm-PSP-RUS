...
#if defined(TARGET_PSP)
    uint32_t palette_key;
    const uint8_t* ci4_palette[16];
    uint32_t ci4_palette_key[16];
#endif
    struct {
        const uint8_t *addr;
        uint8_t fmt;
...
static const uint8_t* gfx_texture_palette_source(int tile) {
    const TextureTileState* tileState = gfx_get_texture_tile(tile);

    if ((tileState->fmt == G_IM_FMT_CI) && (tileState->siz == G_IM_SIZ_4b) &&
        (rdp.ci4_palette[tileState->palette & 0xF] != NULL)) {
        return rdp.ci4_palette[tileState->palette & 0xF];
    }

    return rdp.palette;
}

static uint32_t gfx_texture_palette_source_key(int tile) {
    const TextureTileState* tileState = gfx_get_texture_tile(tile);

    if ((tileState->fmt == G_IM_FMT_CI) && (tileState->siz == G_IM_SIZ_4b) &&
        (rdp.ci4_palette[tileState->palette & 0xF] != NULL)) {
        return rdp.ci4_palette_key[tileState->palette & 0xF];
    }

    return rdp.palette_key;
}

static const uint8_t* gfx_validate_palette_source(int tile, const char* context, uint32_t sizeBytes) {
    const uint8_t* palette = gfx_texture_palette_source(tile);

    if (gfx_normalize_texture_source(&palette, sizeBytes)) {
        return palette;
    }

    gfx_log_bad_texture_source(-1, context, palette, sizeBytes);
    return sInvalidPaletteBuf;
}
...
static bool gfx_texture_cache_lookup(int tile, struct TextureHashmapNode **n, const uint8_t *orig_addr,
    ...
    *uploadInPlace = false;
    ...
    const uint8_t* palette_addr = uses_palette ? gfx_texture_palette_source(tile) : NULL;
    const uint32_t palette_key = uses_palette ? gfx_texture_palette_source_key(tile) : 0;
...
static void import_texture_ci4(int tile) {
...
#if defined(TARGET_PSP)
    const uint8_t* palette = gfx_validate_palette_source(tile, "import_texture_ci4", GFX_CI4_TLUT_SIZE_BYTES);
#else
    const uint8_t* palette = rdp.palette;
#endif
    GfxTextureSwapState paletteSwapState = gfx_texture_source_swap_state(palette, GFX_CI4_TLUT_SIZE_BYTES);
    ...
    for (uint32_t y = 0; y < height; y++) {
        ...
        for (uint32_t x = 0; x < width; x++) {
            ...
            uint16_t col16 = gfx_read_texture_source_be16(palette, idx * 2, &paletteSwapState);
            rgba16_buf[i] = gfx_rgba16_to_gu5551(col16);
        }
    }
...
#if defined(TARGET_PSP)
    const uint8_t* palette = gfx_validate_palette_source(tile, "import_texture_ci8", GFX_TLUT_SIZE_BYTES);
#else
    const uint8_t* palette = rdp.palette;
#endif
    GfxTextureSwapState paletteSwapState = gfx_texture_source_swap_state(palette, GFX_TLUT_SIZE_BYTES);
...
static void gfx_dp_set_tile(uint8_t fmt, uint32_t siz, uint32_t line, uint32_t tmem, uint8_t tile, uint32_t palette, uint32_t cmt, uint32_t maskt, uint32_t shiftt, uint32_t cms, uint32_t masks, uint32_t shifts) {
    ...
    if (gfx_get_render_tile_slot(tile, &renderSlot)) {
        ...
        bool textureImportChanged = (tileState->fmt != fmt) || (tileState->siz != siz) ||
                                    ...
                                    ((tileState->palette != palette) &&
                                     (((tileState->fmt == G_IM_FMT_CI) && (tileState->siz == G_IM_SIZ_4b)) ||
                                      ((fmt == G_IM_FMT_CI) && (siz == G_IM_SIZ_4b))));

        tileState->fmt = fmt;
        tileState->siz = siz;
        tileState->palette = palette & 0xF;
        tileState->cms = cms;
        tileState->cmt = cmt;
...
static bool gfx_loaded_texture_matches_static_cache(int loadSlot, const uint8_t* sourceAddr,
    ...
    return !usesPalette ||
           ((node->palette_key != 0) &&
            (node->palette_addr == gfx_texture_palette_source(loadSlot)) &&
            (node->palette_key == gfx_texture_palette_source_key(loadSlot)));
}
#endif
...
static void gfx_dp_load_tlut(UNUSED uint8_t tile, uint32_t high_index) {
    ...
    bool paletteChanged = (rdp.palette != rdp.texture_to_load.addr) ||
                          (rdp.palette_key != paletteKey);

    /* CI4 SetTile selects one of sixteen 16-entry banks in the RDP TLUT.
     * MM preloads multiple dungeon-map palettes and then selects a bank for
     * each room, so retain every bank touched by a TLUT load. */
    if (rdp.texture_to_load.tmem >= 256) {
        uint32_t destinationEntry = rdp.texture_to_load.tmem - 256;
        uint32_t sourceEntry = 0;
        uint32_t entriesRemaining = high_index + 1;

        while ((destinationEntry < 256) && (entriesRemaining != 0)) {
            const uint32_t bank = destinationEntry >> 4;
            const uint32_t bankOffset = destinationEntry & 0xF;
            uint32_t entriesThisBank = 16 - bankOffset;
            const uint8_t* bankAddr = rdp.texture_to_load.addr + sourceEntry * sizeof(uint16_t);

            if (entriesThisBank > entriesRemaining) {
                entriesThisBank = entriesRemaining;
            }

            const uint32_t bankKey = gfx_texture_palette_key(
                bankAddr, entriesThisBank * sizeof(uint16_t));
            if ((rdp.ci4_palette[bank] != bankAddr) ||
                (rdp.ci4_palette_key[bank] != bankKey)) {
                paletteChanged = true;
            }
            rdp.ci4_palette[bank] = bankAddr;
            rdp.ci4_palette_key[bank] = bankKey;

            destinationEntry += entriesThisBank;
            sourceEntry += entriesThisBank;
            entriesRemaining -= entriesThisBank;
        }
    }
#else
    _UNUSED(high_index);
