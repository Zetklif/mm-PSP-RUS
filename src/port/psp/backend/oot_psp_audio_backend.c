...
static volatile u32 sAudioDiagnosticPcmFrames;
static volatile u32 sAudioDiagnosticPcmNonzeroSamples;
static volatile u32 sAudioDiagnosticPcmPeak;
#endif
...
#if OOT_PSP_AUDIO_DIAGNOSTICS
static void OotPspAudioBackend_RecordPcmSamples(const s16* samples, u32 frames) {
    u32 sampleCount = frames * OOT_PSP_AUDIO_CHANNELS;
    ...
    sAudioDiagnosticPcmFrames += frames;
    sAudioDiagnosticPcmNonzeroSamples += nonzeroSamples;
    if (peak > sAudioDiagnosticPcmPeak) {
        sAudioDiagnosticPcmPeak = peak;
    }
}

static void OotPspAudioBackend_RecordQueuedRingPcm(u32 writePos, u32 frames) {
    while (frames != 0) {
        u32 todo = frames;
        u32 untilWrap = OOT_PSP_AUDIO_RING_FRAMES - writePos;
        ...
        OotPspAudioBackend_RecordPcmSamples(&sAudioRing[writePos * OOT_PSP_AUDIO_CHANNELS], todo);
        frames -= todo;
        writePos = (writePos + todo) & OOT_PSP_AUDIO_RING_MASK;
    }
}
...
#if OOT_PSP_AUDIO_DIAGNOSTICS
    OotPspAudioBackend_RecordQueuedRingPcm(sAudioMePendingQueueWritePos,
                                           sAudioMePendingQueueFrames);
#endif
...
static void OotPspAudioBackend_SnapshotDiagnosticCounters(OotPspAudioDiagnosticCounters* counters) {
    ...
    counters->pcmFrames = sAudioDiagnosticPcmFrames;
    counters->pcmNonzeroSamples = sAudioDiagnosticPcmNonzeroSamples;
    counters->pcmPeak = sAudioDiagnosticPcmPeak;
}
...
static int OotPspAudioBackend_DiagnosticThread(UNUSED SceSize args, UNUSED void* argp) {
    ...
    while (sAudioDiagnosticThreadRunning) {
        ...
        opcodeProfileValid = OotPspAudioBackend_SnapshotMeOpcodeProfile(&opcodeProfile);
        {
            ...
            u32 enabledSequenceMask = 0;
            u32 enabledNotes = 0;
            u32 audioCmdQueueDepth = 0;
            u32 i;
            ...
            for (i = 0; i < (u32)gAudioCtx.audioBufferParameters.numSequencePlayers; i++) {
                if (gAudioCtx.seqPlayers[i].enabled) {
                    enabledSequenceMask |= 1U << i;
                }
            }
            if (gAudioCtx.notes != NULL) {
                for (i = 0; i < (u32)gAudioCtx.numNotes; i++) {
            ...
            if (gAudioCtx.threadCmdProcQueueP != NULL) {
                audioCmdQueueDepth = gAudioCtx.threadCmdProcQueueP->validCount;
            }
            ...
            printf("[audio] buf=%lu/%lu ready=%lu start=%lu recover=%lu ring=%lu meq=%lu driver=%lu "
                   ...
                   (unsigned long)(current.pcmFrames - previousWindow.pcmFrames),
                   (unsigned long)(current.pcmNonzeroSamples - previousWindow.pcmNonzeroSamples),
                   (unsigned long)current.pcmPeak,
            ...
            printf("[audio] phase 1s n=%lu wait_me=%luus prepare=%luus synth=%luus (seq=%luus cmd=%luus) "
                   "submit=%luus abi=%lu dma=%lu | seq=%02lx ids=%02x/%02x/%02x/%02x/%02x notes=%lu "
                   "cmd=%u:%u/q%lu stop=%u\n",
                   ...
                   (unsigned long)((current.profileSampleDmas - previousWindow.profileSampleDmas) /
                                   profileDivisor),
                   (unsigned long)enabledSequenceMask,
                   (unsigned int)gAudioCtx.seqPlayers[0].seqId,
                   (unsigned int)gAudioCtx.seqPlayers[1].seqId,
                   (unsigned int)gAudioCtx.seqPlayers[2].seqId,
                   (unsigned int)gAudioCtx.seqPlayers[3].seqId,
                   (unsigned int)gAudioCtx.seqPlayers[4].seqId,
                   (unsigned long)enabledNotes,
                   (unsigned int)gAudioCtx.threadCmdReadPos,
                   (unsigned int)gAudioCtx.threadCmdWritePos,
                   (unsigned long)audioCmdQueueDepth,
                   (unsigned int)gAudioCtx.threadCmdQueueFinished);
...
#if OOT_PSP_AUDIO_DIAGNOSTICS
    OotPspAudioBackend_RecordPcmSamples(samples, frames);
#endif
...
void OotPspAudio_Init(void) {
    ...
    if (sOotPspAudioInitialized) {
        threadResult = OotPspAudioBackend_StartThreads();
#if OOT_PSP_AUDIO_DIAGNOSTICS
        if (threadResult < 0) {
            printf("[audio!] thread restart failed err=%ld\n", (long)threadResult);
        }
#endif
        return;
    }
    ...
    threadResult = OotPspAudioBackend_StartThreads();
#if OOT_PSP_AUDIO_DIAGNOSTICS
    printf("[audio] startup threads=%ld producer=%d output=%d freq=%lu target=%d/%d/%d updates=%d\n",
           (long)threadResult, (int)sAudioProducerThreadId, (int)sAudioOutputThreadId,
           (unsigned long)gAudioCtx.audioBufferParameters.aiSamplingFreq,
           (int)gAudioCtx.audioBufferParameters.numSamplesPerFrameMin,
           (int)gAudioCtx.audioBufferParameters.numSamplesPerFrameTarget,
           (int)gAudioCtx.audioBufferParameters.numSamplesPerFrameMax,
           (int)gAudioCtx.audioBufferParameters.updatesPerFrame);
#endif
}
